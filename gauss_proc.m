function  gauss_proc( xtrain, ytrain, yreal, xtest, cov_func, sigma)
%gauss_proc example code using the real gaussian process code. 
%    xtrain=sort(rand(1,20)*10-5);
%    xgen=sort(rand(1,10)*10-5);
%    xtest=-5:0.01:5;
%    cov = {'covSum', {'cov_func','covNoise'}};
%    hypgen= [log(1) log(1)];
%    rng(seed)
%    ytrain=ones(size(xgen))*cov_func(hypgen,xgen',xtrain')+.1*randn(size(xtrain));
%    yreal=ones(size(xgen))*cov_func(hypgen,xgen',xtest');
...
%kernel_function = @(x1, x2) x1' * x2; 
   ...
   Ktrain=cov_func(hypSE,xtrain',xtrain')+... %SQUARE EXPONENTIAL
   ...
   %% TEST
   Ktest=cov_func(hypSE,xtrain',xtest')+...    %SQUARE EXPONENTIAL
       covConst(hypconst,xtrain',xtest');%CONSTANT
   % MEAN
   f=Ktest'*alpha; 
   % VARIANCE
   Cov1=cov_func(hypSE,xtest',xtest')+covConst(hypconst,xtest',xtest');
